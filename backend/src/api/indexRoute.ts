...
indexRoute.get("/", async (req, res) => {
  res.json({ Product: convertTypesToFunctionNames(Product.schema.obj) });
});
...
function convertTypesToFunctionNames(obj: { [key: string]: unknown }) {
  ...
  for (const key in obj) {
      result[key] = obj[key]
    if (isPlainObject(obj[key])) result[key] = convertTypesToFunctionNames(obj[key] as { [key: string]: unknown });
    ...
    if (key === "type" && Array.isArray(obj[key]))
      result[key] =
        (obj[key] as Array<unknown>).length === 0
          ? []
          : (obj[key] as Array<unknown>).filter(item => item instanceof Function).map(fnc => (fnc as Function).name)
  }
